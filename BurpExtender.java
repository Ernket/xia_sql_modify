...
public class BurpExtender extends AbstractTableModel implements IBurpExtender, ITab, IHttpListener, IScannerCheck, IMessageEditorController, IContextMenuFactory
{
    ...
    private List<String> whitelistedParams;
    ...
    public void registerExtenderCallbacks(final IBurpExtenderCallbacks callbacks)
    {
        ...
        this.stdout.println("你好 欢迎使用 瞎注!（新增参数白名单）");
        this.stdout.println("version:2.9.1");
        this.stdout.println("原版地址：https://github.com/smxiazi/xia_sql");
        ...
        callbacks.setExtensionName("xia SQL V2.9.1");
        ...
        SwingUtilities.invokeLater(new Runnable()
        {
            ...
            public void run()
            {
                ...
                jps.setLayout(new GridLayout(22, 1)); //六行一列
                ...
                //JButton btn4=new JButton("启动参数白名单");    //参数白名单
                ...
                JButton whitelistButton = new JButton("更新白名单");
                whitelistButton.addActionListener(e -> updateWhitelist());
                ...
                jps.add(textField);
                jps.add(btn3);
                jps.add(new JLabel("参数白名单 (用逗号分隔):"));
                jps.add(whitelistField);
                jps.add(whitelistButton);
                jps.add(jls_4);
                jps.add(chkbox5);
                jps.add(chkbox6);
                jps.add(chkbox7);
    ...
    private void updateWhitelist() {
        ...
        whitelistedParams = Arrays.asList(whitelistText.split("\\s*,\\s*"));
        stdout.println("更新白名单: " + whitelistedParams);
    }
    private void checkVul(IHttpRequestResponse baseRequestResponse, int toolFlag){
        ...
        for (IParameter para : paraList){// 循环获取参数
            ...
            if (para.getType() == 0 || para.getType() == 1 || para.getType() == 6 || para.getType() == is_cookie){ //getTpe()就是来判断参数是在那个位置的
                ...
                // 检查参数是否在白名单中
                if (whitelistedParams != null && whitelistedParams.contains(key)) {
                    stdout.println("跳过白名单参数: " + key);
                    continue;
                }
